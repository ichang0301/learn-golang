...
func (s *SpyStore) Fetch(ctx context.Context) (string, error) {
	...
	go func() {
		var result string
		for _, c := range s.response { // a slow process where we build the result slowly by appending the string, character by character in a goroutine.
			select {
			case <-ctx.Done():
...
type SpyResponseWriter struct { // Sadly httptest.ResponseRecorder doesn't have a way of figuring this out so we'll have to roll our own spy to test for this.
	written bool
}
